  ...
  <body>
    <table class="table" style="width: 73%; border-collapse: collapse; left: 10px; margin-left: 310px">
      ...
      <tbody class="table-group-divider">
        @foreach ($data as $index => $rujukan)
        <tr>
          <td scope="row">{{ $loop->iteration }}</td>
          <td>{{ $rujukan->name }}</td>
          <td>{{ $rujukan->noktp }}</td>
          <td>{{ $rujukan->alamat }}</td>
          <td>{{ $rujukan->keluhan }}</td>
          <td>{{ $rujukan->kecamatan }}</td>
          <td>{{ $rujukan->desa }}</td>
          <td>{{ $rujukan->kodepos }}</td>
          <td>
          <form action="/postSudahDiterima/{{ $rujukan->id }}" method="POST">
              @csrf
              <button type="submit" class="btn btn-outline-primary" name="status" value="Sudah diterima">
                  {{ $rujukan->status }}
              </button>
          </form>
          </td>
          <td>
            <a class="btn btn-outline-success" href="/admin/terimaRujukan/{{ $rujukan->id }}">Terima</a>
          </td>
        </tr>
        @endforeach
      </tbody>
