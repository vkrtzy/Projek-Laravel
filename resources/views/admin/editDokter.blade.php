...
<!-- ======= Header ======= -->
  ...
  <aside id="sidebar" class="sidebar">
    ...
    <ul class="sidebar-nav" id="sidebar-nav">
      ...
      <li class="nav-item">
        <a class="nav-link" data-bs-target="#components-nav" data-bs-toggle="collapse" href="{{ route('admin.dokter') }}">
                ...
                <div class="row">
                    <div class="col d-flex justify-content-center">
                        <div class="card mt-4" style="width: 800px">
                            <div class="card-body">
                                <h5 class="card-title text-center">Edit Dokter</h5>
                                <form action="/postEditDokter/{{ $dokter->id }}" method="POST">
                                    @csrf
                                    <div class="form-group mt-4">
                                        <label class="text-secondary mb-2">Nama</label>
                                        <input type="text" class="form-control border border-secondary form-control" name="name" required value="{{ $dokter -> name }}">
                                    ...
                                    <div class="form-group mt-1">
                                        ...
                                        <input type="text" class="form-control border border-secondary form-control" name="email" required value="{{ $dokter -> email }}">
                                        <span class="text-danger">
                                            @error('email')
                                              {{ $message }}
                                            @enderror
                                    ...
                                    <div class="form-group mt-1">
                                        <label class="text-secondary mb-2">Spesialis</label>
                                        <input type="text" class="form-control border border-secondary form-control" name="spesialis" required value="{{ $dokter -> spesialis }}">
                                        <span class="text-danger">
                                            @error('spesialis')
                                              {{ $message }}
                                            @enderror
                                    ...
                                    <button type="submit" class="btn btn-success mt-5">Edit Data Dokter</button>
                                </form>
                            </div>
                        </div>
                    </div>
